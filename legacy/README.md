> **⚠️ Archived — legacy single-task implementation.**
> This folder preserves the original single-task FoodFlow GNN code, where a
> separate GAT/GCN checkpoint was trained per SCTG commodity code
> (`best_model1_gcn.pth` … `best_model7_gcn.pth`). It has been superseded by
> the multi-task model in `../code/`, which predicts all 7 SCTG categories
> with a single `MultitaskSparseGCN`. This code is kept for reference only and
> is not guaranteed to run against the current top-level layout.
>
> The document below is the original README for that single-task version.
